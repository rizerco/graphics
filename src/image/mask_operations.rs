use crate::composite::{self, Layer, Operation};
use crate::{BlendMode, Image, Point, PositionedMask};

/// Deletes the pixels in the image within the supplied mask image.
pub fn delete_pixels(image: &mut Image, mask: &PositionedMask) {
    let base_layer = Layer::new(image, Point::zero());
    let mut blend_layer = Layer::new(&mask.image, mask.origin.into());
...
pub fn subimage(image: &Image, mask: &PositionedMask) -> Image {
    let base_origin = mask.origin * -1;
    ...
    let operation = Operation::new(vec![base_layer, blend_layer], mask.image.size.into());
...
mod tests {
    ...
    use crate::{Image, PositionedMask, Rect};
    ...
    fn delete_pixels() {
        ...
        let mask = PositionedMask::bounded(mask_image, bounding_box);
